...
const DATA = {
  ...
  skills: [
    ...
    },
    {
      icon: "🏆",
      name: "Certifications",
      list: "CompTIA Security+ · CCNA (Routing & CyberOps) · AWSML Essentials",
    },
  ],
  achievements: [
    ...
    const init = async () => {
      ...
      const interval = setInterval(() => {
        ...
        if (prog >= 100) {
          ...
          setTimeout(() => {
            gsap.to(loader, { yPercent: -100, duration: 1.1, ease: "power4.inOut", onComplete: () => { if (loader) loader.style.display = "none"; } });
            gsap.to("nav", { opacity: 1, duration: 0.6, delay: 0.4 });
            gsap.to(".hero-tag", { opacity: 1, y: 0, duration: 0.7, delay: 0.6 });
            gsap.to(".hero-name .word", { y: 0, duration: 0.9, ease: "power4.out", stagger: 0.08, delay: 0.75 });
      ...
      const mat = new THREEmod.PointsMaterial({ color: 0x0d9488, size: 0.18, transparent: true, opacity: 0.65, sizeAttenuation: true });
      const points = new THREEmod.Points(geo, mat);
      scene.add(points);
      const glowGeo = new THREEmod.SphereGeometry(12, 32, 32);
      const glowMat = new THREEmod.MeshBasicMaterial({ color: 0x0d9488, transparent: true, opacity: 0.04, wireframe: false });
      const glowMesh = new THREEmod.Mesh(glowGeo, glowMat);
      scene.add(glowMesh);
      let targetX = 0, targetY = 0;
      const onMouseMove3D = (e: MouseEvent) => { targetX = (e.clientX / window.innerWidth - 0.5) * 0.8; targetY = (e.clientY / window.innerHeight - 0.5) * 0.8; };
      window.addEventListener("mousemove", onMouseMove3D);
      let tick = 0;
      const animate = () => { animId = requestAnimationFrame(animate); tick += 0.003; points.rotation.y = tick * 0.04 + targetX * 0.3; points.rotation.x = tick * 0.02 - targetY * 0.2; glowMesh.rotation.y = tick * 0.07; renderer.render(scene, camera); };
      animate();
      const onResize = () => { camera.aspect = window.innerWidth / window.innerHeight; camera.updateProjectionMatrix(); renderer.setSize(window.innerWidth, window.innerHeight); };
      window.addEventListener("resize", onResize);
      const EASE = "power3.inOut";
      gsap.utils.toArray<HTMLElement>(".section-label").forEach((el) => {
        gsap.to(el, { opacity: 1, y: 0, duration: 0.7, ease: EASE, scrollTrigger: { trigger: el, start: "top 85%" } });
      });
      document.querySelectorAll<HTMLElement>(".section-title").forEach((title) => {
        const chars = title.querySelectorAll<HTMLElement>(".char");
        if (chars.length > 0) { gsap.to(chars, { y: 0, duration: 0.8, ease: "power4.out", stagger: 0.015, scrollTrigger: { trigger: title, start: "top 85%" } }); }
      });
      gsap.utils.toArray<HTMLElement>(".about-bio p").forEach((p, i) => {
        gsap.to(p, { opacity: 1, y: 0, duration: 0.7, ease: EASE, delay: i * 0.12, scrollTrigger: { trigger: p, start: "top 88%" } });
      });
      gsap.utils.toArray<HTMLElement>(".stat-box").forEach((box, i) => {
        gsap.to(box, { opacity: 1, y: 0, duration: 0.6, ease: EASE, delay: i * 0.1, scrollTrigger: { trigger: box, start: "top 88%" } });
      });
      gsap.utils.toArray<HTMLElement>(".exp-item").forEach((item, i) => {
        gsap.to(item, { opacity: 1, x: 0, duration: 0.7, ease: EASE, delay: i * 0.07, scrollTrigger: { trigger: item, start: "top 88%" } });
      });
      gsap.utils.toArray<HTMLElement>(".project-item").forEach((item, i) => {
        gsap.to(item, { opacity: 1, x: 0, duration: 0.7, ease: EASE, delay: i * 0.08, scrollTrigger: { trigger: item, start: "top 88%" } });
      });
      gsap.utils.toArray<HTMLElement>(".skill-card").forEach((card, i) => {
        gsap.to(card, { opacity: 1, y: 0, duration: 0.6, ease: EASE, delay: (i % 3) * 0.08, scrollTrigger: { trigger: card, start: "top 90%" } });
      });
      gsap.to(".contact-sub", { opacity: 1, duration: 0.7, ease: EASE, scrollTrigger: { trigger: ".contact-sub", start: "top 88%" } });
      gsap.to(".contact-email", { opacity: 1, duration: 0.7, ease: EASE, scrollTrigger: { trigger: ".contact-email", start: "top 88%" } });
      gsap.to(".social-row", { opacity: 1, duration: 0.7, ease: EASE, scrollTrigger: { trigger: ".social-row", start: "top 90%" } });
      cleanup = () => { cancelAnimationFrame(animId); window.removeEventListener("mousemove", onMove); window.removeEventListener("mousemove", onMouseMove3D) ; window.removeEventListener("resize", onResize); lenis.destroy(); renderer.dispose(); geo.dispose(); mat.dispose(); ScrollTrigger.getAll().forEach((st) => st.kill()); };
    };
    init();
    return () => cleanup?.();
  }, []);
  ...
  const splitChars = (text: string) => {
    ...
    return words.map((w, wi) => (
      <span className="word" key={wi}>
        {w.split("").map((c, ci) => (<span className="char" key={ci}>{c}</span>))}
  ...
  return (
    <>
      ...
      <div id="about" className="section">
        ...
        <div className="about-grid">
          ...
          <div className="stats-grid">
            {DATA.stats.map((s, i) => (<div className="stat-box" key={i}><div className="stat-num">{s.num}</div><div className="stat-lbl">{s.lbl}</div></div>))}
          </div>
        </div>
      </div>
      <div id="experience" className="exp-section">
        ...
        <div className="exp-list">{DATA.experience.map((e, i) => (<div className="exp-item" key={i}><div className="exp-date">{e.date}</div><div><div className="exp-company">{e.company}</div><div className="exp-role">{e.role}</div><div className="exp-desc">{e.desc}</div></div></div>))}</div>
      </div>
      <div id="projects" className="projects-section">
        <div className="section-label">03 — Projects</div>
        <h2 className="section-title">{splitChars("What I've Built")}</h2>
        <div>{DATA.projects.map((p, i) => (<a href={p.link} target="_blank" rel="noopener noreferrer" className="project-item" key={i} style={{ display: "grid", textDecoration: "none" }}><span className="proj-num">0{+ 1}</span><div><div className="proj-name">{p.name}</div><div className="proj-desc">{p.desc}</div><div className="proj-tags" style={{ marginTop: "1rem" }}>{p.tags.map((t, j) => (<span className="proj-tag" key={j}>{t}</span>))}</div></div><div className="proj-arrow">↗</div></a>))}</div>
      </div>
      <div id="skills" className="skills-section">
        <div className="section-label">04 — Skills</div>
        <h2 className="section-title">{splitChars("What I Know")}</h2>
        <div className="skills-grid">{DATA.skills.map((s, i) => (<div className="skill-card" key={i}><div className="skill-icon">{s.icon}</div><div className="skill-name">{s.name}</div><div className="skill-list">{s.list}</div></div>))}</div>
        <div style={{ marginTop: "4rem" }}><div className="section-label" style={{ opacity: 1, marginBottom: "1.5rem" }}>Achievements & Recognition</div><el style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: ".9rem" }}>{DATA.achievements.map((a, i) => (<li key={i} style={{ fontSize: ".92rem", color: "var(--muted)", paddingLeft: "1.2rem", borderLeft: "2px solid var(--teal-dim)" }}><span style={{ color: "var(--teal)", marginRight: ".5rem" }}>覒</span>{a}</li>))}</el></div>
      </div>
      <div id="contact" className="contact-section">
        ...
        <p className="contact-sub">Open to full-time cybersecurity roles, security engineering positions, and consulting.</p>
        <a href={`mailto:${DATA.email}`} className="contact-email">{DATA.email}</a>
        <div className="social-row">
          <a href={DATA.github} target="_blank" rel="noopener noreferrer"><span>⊭</span> GitHub</a>
          <a href={DATA.linkedin} target="_blank" rel="noopener noreferrer"><span>↰</span> LinkedIn</a>
          <a href={`tel:${DATA.phone}`}><span>✆</span> {DATA.phone}</a>
        </div>
      </div>
      <footer><span>© 2026 Malaiyappan S</span><span>Built with Next.js · Three.js · GSAP</span></footer>
    </>
  );
}
um">0{i + 1}</span>
